...
import { applyPanelFeedbackRetailor } from '@/lib/ai/apply-panel-feedback-retailor'
...
const panelRetailorSchema = z.object({
...
export async function POST(request: Request) {
  ...
  const rateLimit = await checkRateLimit('panel-retailor', ip)
  ...
  try {
    ...
    const parsed = panelRetailorSchema.safeParse(body)

    if (!parsed.success) {
      return NextResponse.json({ error: 'Invalid request.' }, { status: 400 })
    }

    const { jobDescription, sourceResumeText, draft, panel } = parsed.data

    const panelRun = await applyPanelFeedbackRetailor({
      jobDescription,
      sourceResumeText,
    ...
    return NextResponse.json(
      { error: safeErrorMessage(error, 'Failed to de-escalate and re-tailor from panel feedback.') },
      { status: 500 }
    )
  }
}
